...
/**Memory Access. Any instruction requiring values stored in the memory dealt here. Others just passed on to the next stage.**/
		...
		if(ExMem.at(0) == 0){
			...
			data_memory[address] = result;			//Saving data to the data memory.
			MemWb.push_back(ExMem.at(0));						
		...
		if(IfId.size()>1){
			...
			*/
			...
			/*Control Hazard Detection. For all the branching instructions, the pipeline is stalled for 5 five cycles or in
			...
			if(opcode == 0){
				...
				ss >> offset;
//				cout<<"rs "<<rs<<" value "<<file_array[29]<<endl;
